...
def load_yolo_model():
    model = YOLO("yolov8n.pt")  # Load the YOLOv8 model (you can change to yolov8s.pt, yolov8m.pt, etc.)
    return model
    ...
    for result in results:
        for box in result.boxes:
            ...
            if confidence > 0.5:  # Confidence threshold
                detections.append((label, confidence, (int(x1), int(y1), int(x2 - x1), int(y2 - y1))))
    return detections
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.write("Developed by: G.Sampath")
        st.markdown("[GitHub](https://github.com/sampath123123) | [LinkedIn](https://www.linkedin.com/in/gugulothu-sampath-829b33299)")
        st.markdown("---")
